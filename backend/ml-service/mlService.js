...
4. TECHNICAL ACCURACY: Technical terms like 'compiler', 'iot', 'RFID', 'Lexical Analysis' must be spelled and formatted correctly.
5. NO HALLUCINATION: If a word is truly illegible, use "[...]" instead of guessing.
...
export const generateQuestionsFromText = async (text, difficulty = "Medium", questionCount = 5, topic = "") => {
    try {
        ...
        const prompt = `You are an elite examiner. Generate exactly ${questionCount} MCQs about "${targetTopic}".
        Difficulty Level: ${targetDiff}.
        ${text && text.length > 50 ? `Context Material: \n\n${text}\n\n` : `Provide expert questions based on general knowledge for "${targetTopic}".`}
        ...
        Return ONLY valid JSON: {"questions": [{"topic": "${targetTopic}", "difficulty": "${targetDiff}", "questionText": "...", "options": ["A","B","C","D"], "correctAnswer": "...", "explanation": "..."}]}`;
    ...
    try {
        ...
        const response = await axios.post('https://api.groq.com/openai/v1/chat/completions', {
            ...
            messages: [
                { 
                    role: 'system', 
                    content: `You are an expert academic evaluator. Your task is to provide a "STRONGLY REASONED" assessment of a student's answer.
                    
                    RULES:
                    1. If the answer is WRONG or POOR (score < 50): Use a firm, "downgrade" tone. Explain exactly what fundamental concept was missed. Be critical but constructive.
                    2. If the answer is GOOD/PERFECT (score >= 80): Use an encouraging, "accurate and professional" tone. Highlight the strength of their reasoning.
                    3. Always provide a detailed "stepAnalysis" array of strings showing the logical progression of the correct solution versus the user's approach.
                    
                    RETURN JSON ONLY: 
                    {
                        "isCorrect": boolean, 
                        "score": number (0-100), 
                        "feedback": "Deeply reasoned explanation with appropriate tone",
                        "stepAnalysis": ["Step 1...", "Step 2..."]
                    }` 
                },
                { role: 'user', content: `Question: ${question}\n${options?.length > 0 ? `Options: ${options.join(' | ')}\n` : ''}Expected Correct Answer: ${correctAnswer}\nStudent's Submitted Answer: ${userAnswer}` }
            ],
            response_format: { type: "json_object" }
        }, { headers: { 'Authorization': `Bearer ${groqApiKey}` }, timeout: 15000 });
